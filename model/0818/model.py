...
def f_rate_2(y, t, para):
    ...
    D = Config.D
    dn1_dt = Config.k_act*n_CH4 - Config.k_deact*n1 - sigma*D*n1*(2*n1+a2+a_s+a_d) +\
             delta*a2*2
    da2_dt = sigma*D*n1*2*n1 - sigma*D*n1*2*a2 - delta*a2*2
    dns_dt = sigma*D*n1*a2
    das_dt = (3*sigma*D*n1*a2 + sigma*D*n1*a_s)*Config.K
    dad_dt = sigma*D*n1*a_d * Config.K
    dtheta_dt = (dn_CH4_dt + dn1_dt + da2_dt + das_dt + dad_dt) / Config.n0   # will lose count of some atom. Larger than real.
...
def f_rate_4(y, t, para):
    ...
    dad_dt = sigma*D*n1*a_d * Config.K
    dtheta_dt = (dn_CH4_dt + dn1_dt + da2_dt + da3_dt + da4_dt + das_dt + dad_dt) / Config.n0   # will lose count of some atom. Larger than real.
