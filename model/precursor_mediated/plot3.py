...
y0 = [0, 0, 0, 0, 0, 0, 0]
sigma = 1e-10
para = (sigma, sigma, sigma, sigma, sigma, Config.decay_rate,Config.decay_rate*1e-1)
t = np.linspace(0, 1e-5, 10000)
sol = odeint(f_rate3, y0, t, args=(para,))
# plot results
fig0 = plt.figure()
...
if use_log_scale is True:
    ...
    ax0.loglog(t, sol[:, 2], 'g-', label='#Unstable cluster')
    ax0.loglog(t, sol[:, 3], 'k-', label='#Stable cluster')
    ...
    ax0.plot(t, sol[:, 2], 'g-', label='#Unstable cluster')
    ax0.plot(t, sol[:, 3], 'k-', label='#Stable cluster')
...
if use_log_scale is True:
    ...
    ax1.loglog(t, sol[:, 3], 'k-', label='#Stable cluster')
    ...
    ax1.plot(t, sol[:, 2], 'g-', label='#Unstable cluster')
    ax1.plot(t, sol[:, 3], 'k-', label='#Stable cluster')
...
sol = odeint(f_rate3, y0, t, args=(para,), mxstep=100000)
# plot results
fig2 = plt.figure()
...
if use_log_scale is True:
    ...
    ax2.loglog(t, sol[:, 2], 'g-', label='#Unstable cluster')
    ...
    ax21.loglog(t, sol[:, 6], 'mo', label='Coverage')
else:
